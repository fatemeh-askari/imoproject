...
from .models import Item, Request, SelectedItem, RequestDetail
...
#     selected_items = SelectedItem.objects.filter(request=current_request).values_list('item_id', flat=True)
#
#     # ایجاد فرم یا پردازش آن
#     if request.method == 'POST':
#         form = RequestDetailForm(request.POST)
#         if form.is_valid():
#             request_detail = form.save(commit=False)
#             request_detail.request = current_request  # مرتبط کردن جزئیات با درخواست جاری
#             request_detail.save()
#             return redirect('item_list_for_request', request_id=request_id)
#     else:
...
def item_list_for_request(request, request_id):
    ...
    # گرفتن جزئیات مرتبط با درخواست
    request_details = RequestDetail.objects.filter(request=current_request)
    ...
    return render(request, 'Itemsapp/items_list.html', {
        ...
        'current_request': current_request,
        'form': form,  # ارسال فرم به قالب
        'request_details': request_details,  # ارسال جزئیات به قالب
    })

from django.http import JsonResponse
from django.shortcuts import get_object_or_404
from .models import SelectedItem, Request
...
from django.http import JsonResponse
from django.shortcuts import get_object_or_404
from .models import SelectedItem, Item
...
# def get_selected_items(request, request_id):
#     selected_items = SelectedItem.objects.filter(request_id=request_id).select_related('item')
#     data = [{"id": si.item.id, "title": si.item.title} for si in selected_items]
...
def get_selected_items(request, request_id):
    selected_items = SelectedItem.objects.filter(request_id=request_id).select_related('item').distinct()
...
import json
from django.http import JsonResponse
from .models import Request, Item, SelectedItem
...
from django.http import JsonResponse
from django.shortcuts import get_object_or_404
from .models import SelectedItem, Item, Request

# def update_item_selection(request, request_id, item_id):
#     # دریافت درخواست فعلی
#     current_request = get_object_or_404(Request, id=request_id)
#     item = get_object_or_404(Item, id=item_id)
#
#     # بررسی اینکه آیتم قبلاً انتخاب شده است یا خیر
#     existing_item = SelectedItem.objects.filter(request=current_request, item=item)
#
#     if existing_item.exists():
#         # اگر آیتم قبلاً انتخاب شده باشد، هیچ کاری انجام نمی‌دهیم
#         return JsonResponse({'success': False, 'message': 'Item already selected'})
#
#     # در غیر این صورت، آیتم را به لیست اضافه می‌کنیم
#     SelectedItem.objects.create(request=current_request, item=item)
#
#     # داده‌های جدید را برای پاسخ ارسال می‌کنیم
#     selected_items = SelectedItem.objects.filter(request=current_request)
#     selected_items_data = [{"id": selected_item.id, "title": selected_item.item.title} for selected_item in selected_items]
#
#     return JsonResponse({'success': True, 'message': 'Item added successfully', 'selected_items': selected_items_data})
...
#     item = get_object_or_404(Item, id=item_id)
#
#     action = request.POST.get('action')  # دریافت اکشن (add/remove)
#
#     if action == 'add':
#         # بررسی اینکه آیتم قبلاً انتخاب شده است یا خیر
#         existing_item = SelectedItem.objects.filter(request=current_request, item=item)
...
#     selected_items_data = [{"id": selected_item.id, "title": selected_item.item.title} for selected_item in selected_items]
#
#     return JsonResponse({'success': True, 'message': 'Item updated successfully', 'selected_items': selected_items_data})
...
def update_item_selection(request, request_id, item_id):
    ...
    # دریافت داده‌های JSON از بدن درخواست
    try:
    ...
    # ارسال داده‌های جدید به‌روزرسانی شده
    selected_items = SelectedItem.objects.filter(request=current_request)
    selected_items_data = [{"id": selected_item.id, "title": selected_item.item.title} for selected_item in selected_items]
...
from django.shortcuts import get_object_or_404, redirect
from .models import RequestDetail, Request

from django.shortcuts import get_object_or_404, redirect

def delete_request_details(request, request_id, detail_id):
    current_request = get_object_or_404(Request, id=request_id)
    request_detail = get_object_or_404(RequestDetail, id=detail_id, request=current_request)
...
def pdf_view(request, request_id):
    ...
    for selected_item in selected_items:
        if selected_item.item.title not in seen_titles:
            seen_titles.add(selected_item.item.title)
            unique_selected_items.append(selected_item)
...
def add_new_item(request, request_id):
    current_request = get_object_or_404(Request, id=request_id)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            # برگشت پاسخ به صورت JSON
            return JsonResponse({'success': True, 'item': form.cleaned_data})  # ارسال داده‌های فرم به عنوان پاسخ
        else:
