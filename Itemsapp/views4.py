...
def add_request_detail(request, request_id):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            new_detail.request = request_instance
            new_detail.save()
            return redirect('request_detail_view', request_id=request_id)  # بازگشت به صفحه نمایش جزئیات درخواست
    else:
        form = RequestDetailForm()
    existing_details = RequestDetail.objects.filter(request=request_instance)
